...
1. A set of community-developed **standardized vocabularies** and a specified process for how other
...
HED standard schema has the globally unique identifier ***HED_term***. Similarly, each term in a library schema
is uniquely identified by ***HED-libraryName_term***. The HED vocabulary terms are permanent in the sense that,
...
The schemas are retrievable by tools or humans through GitHub. A [schema viewer](https://www.hedtags.org/display_hed.html) is available to view
...
a widely-endorsed standard for clinical annotation of EEG, has been translated into a [HED-Score](https://hed-schemas.readthedocs.io/en/latest/hed_score_schema.html#) library schema,
...
has a fairly complete set of language-independent [valid/invalid test cases](https://github.com/hed-standard/hed-specification/tree/master/tests/json_tests) specified in JSON.
...
tag summaries in its dataset summaries as seen in this [beta version](https://nemar.org/dataexplorer/detail?dataset_id=ds003645) of the interface.
