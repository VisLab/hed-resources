...
More detailed information about the remodeling operations can be found in the [**HED remodeling tools**](hed-remodeling-tools-anchor) guide.
...
Each remodel operation has its own specific set of required parameters that can be found under [**HED remodeling tools**](./HedRemodelingTools.md). For *rename_columns*, the required operations are *column_mapping* and *ignore_missing*. Some operations also have optional parameters.
...
This section discusses a complex example using the [**sub-0013_task-stopsignal_acq-seq_events.tsv**](./_static/data/sub-0013_task-stopsignal_acq-seq_events.tsv) events file of AOMIC-PIOP2 dataset available on [OpenNeuro](https://openneuro.org) as ds002790. Here is an excerpt of the event file.
...
The final remodeling file can be found at: [**finished json remodeler**](./_static/data/AOMIC_splitevents_rmdl.json)
